...
function buildAHorse(num) {
    ...
    let ear = 0;
    if (num[4] <= 1) { // 2 in 10 chance
        ear = 0;
    } else if (num[4] <= 4) { // 3 in 10 chance
        ear = 1;
    } else { // 5 in 10 chance
        ear = 2;
    }
    ...
    if (num[5] == 0) { // 1 in 10
        hair = 0;
    } else if (num[5] <= 2) { // 2 in 10
        hair = 1;
    } else if (num[5] <= 5) { // 3 in 10
        hair = 2;
    } else { // 4 in 10
        hair = 3;
    }
    ...
    // 0 1 3 5 6 8 A C E F
    let chars = ['0', '1' , '3', '5', '6', '8', 'A', 'C', 'E', 'F']

    let hairColor = '#';
    for (let i = 6; i < 12; i++) {
        hairColor += chars[parseInt(num[i])];
    }
    console.log('hair color: ' + hairColor);

    let lineColors = '#';
    for (let i = 12; i < 18; i ++) {
        lineColors += chars[parseInt(num[i])];
    }
    console.log('line color: ' + lineColors);

    let horse = '<svg id="Layer_1" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 300 300"><defs><style>.cls-1,.cls-2,.cls-4 {fill: ';
    ...
    horse += '}.cls-1,.cls-2,.cls-3,.cls-4,.cls-5{stroke:'

    if (color == 'white') { // stroke
        horse += "#000";
    } else {
        horse += "#fff";
    }

    horse += '}.cls-1,.cls-2,.cls-3,.cls-4,.cls-5,.cls-6,.cls-7{stroke-miterlimit:10}.cls-1{stroke-width:6px}.cls-2,.cls-3{stroke-width:5px}.cls-3{fill:';
    horse += hairColor;
    horse += '}.cls-4{stroke-width:7px}.cls-6,.cls-7{fill:'
    horse += lineColors + ';stroke:' + lineColors;
    horse += '}.cls-6{stroke-width:4px}</style></defs>';
    horse += horseParts.backEars[ear];
    if (horn) {horse += horseParts.horn};
    horse += horseParts.hairs[hair];
    horse += horseParts.body;
    horse += horseParts.base;
    horse += horseParts.frontEars[ear];
    horse += horseParts.eye.shape;
    horse += (color == 'white' ? horseParts.eye.white : horseParts.eye.black);
    horse += horseParts.nostril.shape;
    horse += (color == 'white' ? horseParts.nostril.white : horseParts.nostril.black);
    horse += '<path class="cls-6" d="M116.54,212.41c-.42-.72,11.36-6.42,23.94-18.24,5-4.7,10.42-9.8,14.82-17.1,11.53-19.16,7.75-39.9,9.12-39.9,1.13,0,3.22,11.55,1.14,23.94A52,52,0,0,1,161,175.93c-4,8.13-9.07,12.88-14.82,18.24a94.67,94.67,0,0,1-17.1,12.54C123.35,210,116.84,213,116.54,212.41Z" /><path class="cls-6" d="M215.68,196.24c2.59-15.36,4.13-23.45,3.56-35.62-.25-5.42-.87-10.75-1.78-18.41-2.18-18.54-4.1-26-3.56-26.12.83-.2,8.06,17.9,9.5,40.37a122.52,122.52,0,0,1-1.19,27.31c-1,6.23-2,9.3-4.15,17.22-6.14,23-8.55,38.66-8.91,38.6S212.36,216,215.68,196.24Z" /><path class="cls-7" d="M149.85,115.23a39.09,39.09,0,0,1-9.5,14.25c-2.85,2.79-8.44,8.27-16.63,9.5a25.86,25.86,0,0,1-11.87-1.18c0,.12,9.09,4.78,17.81,2.37C144,136.21,149.84,115.23,149.85,115.23Z" /></svg>'
